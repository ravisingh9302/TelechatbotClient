...
function Chat({ adminId, visitorId, host, config }) {
    ...
    return (
        ...
        <div className=' flex flex-col  max-h-screen h-screen overflow-hidden'>
            ...
            <div className='' >
                <input id='inputtg' className={`outline-none z-50 px-1 ${config?.placeholderTxtcolor?config.placeholderTxtcolor:'placeholder:text-gray-500'} mb-[2px]  h-9  min-h-10 max-h-20 w-full font-medium`} style={{ color: config?.inputTxtcolor ? config.inputTxtcolor : "#000000", backgroundColor: config?.inputBgcolor ? config.inputBgcolor : "#ffffff" }} type="text" placeholder={config?.placeholderText ? config.placeholderText : "Type your message..."} value={input} onChange={(e) => setinput(e.target?.value)} onKeyDown={handlesubmit} />
            </div>
        </div>
